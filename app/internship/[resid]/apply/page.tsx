// app/internship/[resid]/apply/page.tsx
"use client";

import { useState, useEffect } from "react";
import { useRouter, useParams } from "next/navigation";
import { doc, getDoc, collection, addDoc, updateDoc } from "firebase/firestore";
// Import the new firebase configuration for the huge data project
...
interface InternshipData {
  id: string;
  company?: string;
  responseSchema?: ResponseSchema; // This field holds the dynamic form configuration.
  // ... other fields as needed
}

// Pre-generate static pages for each internship document by its document ID
...
export default function ApplyPage() {
  ...
  useEffect(() => {
    async function fetchInternship() {
      try {
        const docRef = doc(dbHugeData, "internships", resid);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get a reference to the internship document.
      const internshipDocRef = doc(dbHugeData, "internships", internship.id);
      // Add a new document to the "responses" subcollection under the internship.
      const responsesCollectionRef = collection(internshipDocRef, "responses");
      await addDoc(responsesCollectionRef, applicationData);

      // Optionally, update a specific key in the internship document (e.g., "response")
      // if you want to maintain a quick summary. Uncomment and adjust if needed:
